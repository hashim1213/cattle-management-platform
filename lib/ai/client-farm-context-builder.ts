/**
 * Client-side Farm Context Builder
...
export class ClientFarmContextBuilder {
  ...
  private async getCattleContext() {
    try {
      ...
      console.error("Error getting cattle context:", error)
      return { all: [], total: 0, byStatus: {}, byPen: {}, avgWeight: 0, healthIssues: [], recent: [] }
    }
  }
  ...
  private async getPensContext() {
    try {
      ...
      console.error("Error getting pens context:", error)
      return { total: 0, utilization: 0, available: 0, overcrowded: [], empty: [] }
    }
  }
  ...
  private async getBarnsContext() {
    try {
      ...
      console.error("Error getting barns context:", error)
      return { total: 0, list: [] }
    }
  }
  ...
  private async getInventoryContext() {
    try {
      ...
      return { all: [], total: 0, totalValue: 0, lowStock: [], recent: [] }
    }
  }
  ...
  private async getHealthContext() {
    try {
      ...
      // Get first 50 cattle docs for health records
      ...
      return { recent: [], activeCases: 0, mostUsed: [] }
    }
  }
  ...
  private async getActivitiesContext() {
    try {
      ...
      console.error("Error getting activities context:", error)
  ...
  private getEmptyContext(): FarmContext {
    return {
      cattle: { total: 0, byStatus: {}, byPen: {}, recentAdditions: [], healthIssues: [], avgWeight: 0 },
      pens: { total: 0, utilization: 0, available: 0, overcrowded: [], empty: [] },
      barns: { total: 0, list: [] },
      inventory: { total: 0, lowStock: [], totalValue: 0, recentUsage: [] },
      health: { recentTreatments: [], activeCases: 0, mostUsedMedications: [] },
      activities: { recent: [], byType: {} },
      financials: { totalPurchaseValue: 0, inventoryValue: 0, avgCattlePrice: 0 },
      summary: "No farm data available yet. Start by adding cattle, pens, or inventory items.",
    }
  }
}
